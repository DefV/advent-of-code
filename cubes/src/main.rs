...
fn main() {
    ...
    let sum = games.iter().filter(|game| game.possible()).fold(0, |sum, game| sum + game.id);
...
impl<'g> Game<'g> {
    fn possible(&self) -> bool {
        self.turns.iter().all(|turn| 
            POSSIBLE_CONTENTS.iter().all(|(color, count)| 
                turn.get(color).unwrap_or(&0) <= count
            )
        )
    }
}
...
fn process_line(line: &str) -> Game {
    ...
    let mut game = Game { id, turns: Vec::new() };
...
mod tests {
    ...
    fn test_process_line() {
        ...
        assert_eq!(game.turns.iter().map(|t| t.len()).collect::<Vec<_>>(), [2, 3, 1]);
    ...
    fn test_possible_with_possible() {
        let possible_game = Game { id: 1, turns: vec![
            [("red", 12), ("green", 13), ("blue", 14)].iter().cloned().collect(),
            [("red", 12), ("blue", 14)].iter().cloned().collect(),
            [("red", 10), ("green", 13), ("blue", 14)].iter().cloned().collect(),
        ]};
    ...
    fn test_possible_with_impossible() {
        let impossible_game = Game { id: 1, turns: vec![
            [("red", 12), ("green", 13), ("blue", 14)].iter().cloned().collect(),
            [("red", 12), ("green", 13), ("blue", 15)].iter().cloned().collect(),
            [("red", 12), ("green", 13), ("blue", 13)].iter().cloned().collect(),
        ]};
